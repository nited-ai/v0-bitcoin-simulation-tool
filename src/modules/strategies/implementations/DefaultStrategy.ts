 ...
 * 
 * Independent implementation of the default investment strategy.
 * Maintains the original behavior of the simulation with no additional restrictions.
 * Invests up to target LTV when BTC accumulation is enabled.
 */
...
export class DefaultStrategy implements InvestmentStrategyInterface {
  ...
  makeDecision(context: StrategyContext): StrategyDecision {
    ...
    // Use debt capacity from context (already calculated)
    ...
    // Handle edge case: negative or zero debt capacity
    if (debtCapacity <= 0) {
      return {
        ...
        withdrawalAmount: 0,
        reasoning: "Insufficient debt capacity - cannot take any loans"
      }
    }
