...
describe('PriceUpdater.fillGaps', () => {
  ...
  it('writes lastSuccessfulCronAt heartbeat after successful fillGaps', async () => {
    const store = mockStore({
      getLatest: vi.fn().mockResolvedValue({ date: '2026-05-01' } as any),
    })
    const fetchHistory = vi.fn().mockResolvedValue([{ ...fakePoint, date: '2026-05-02' }])
    const updater = createPriceUpdater({ store, fetchCurrent: vi.fn(), fetchHistory })
  ...
  it('throws if fetchHistory not configured', async () => {
    const store = mockStore({ getLatest: vi.fn().mockResolvedValue({ date: '2026-05-01' } as any) })
    const updater = createPriceUpdater({ store, fetchCurrent: vi.fn() })

    await expect(updater.fillGaps()).rejects.toThrow(/fetchHistory not configured/)
  })
})
